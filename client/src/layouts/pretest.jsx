  ...
  const getPretestData = async () => {
    try {
      const res = await fetch(
        `http://${LOCAL_HOST}:${LOCAL_PORT}/getSimulation?broadId=${experimentIds?.broadId}&labId=${experimentIds?.labId}&expId=${experimentIds?.expId}&isDocument=true&documentName=pretest`,
        { method: "GET" }
      );
                    ...
                    style={{
                      color:
                        ...
                        selectedAnswers[index] === key
                          ? "green" // Highlight correct answer in green when selected
                          : showResult &&
                            selectedAnswers[index] !== question.correctAnswer &&
                            selectedAnswers[index] === key
                          ? "red" // Highlight incorrect answer in red when selected
                          : "inherit",
                    }}
                  >
                    {value}
...
const PretestComponent = WithExperimentLayout(Pretest);
